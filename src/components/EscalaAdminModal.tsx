...
export function EscalaAdminModal({
  ...
  // Mock coverage index based on ideal vs min until generator is integrated
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] p-0 overflow-hidden flex flex-col">
        <DialogHeader className="p-6 pb-2 shrink-0">
          <DialogTitle className="text-2xl font-bold flex items-center gap-2">
            ...
            Painel de Governança de Escalas
          </DialogTitle>
          <DialogDescription>
            Administração centralizada de ciclos, setores, contratos e métricas.
          </DialogDescription>
        </DialogHeader>

        <Tabs defaultValue="ciclos" className="flex-1 flex flex-col overflow-hidden">
          <div className="px-6 shrink-0">
            <TabsList className="grid grid-cols-1 md:grid-cols-4 w-full h-auto min-h-12 py-1">
                    ...
                    <TableBody>
                      {cycles.map((c) => (
                        <TableRow key={c.id}>
                          ...
                          <TableCell>
                            {format(new Date(c.request_deadline), 'dd/MM/yyyy')}
                          </TableCell>
                          <TableCell>
                            <Badge
                              variant={
