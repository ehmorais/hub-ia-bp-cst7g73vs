...
function AutoGenerateInner({
  ...
  const handleExecutePdfExport = async (formatType: 'daily_list' | 'calendar') => {
    setIsPdfExportModalOpen(false)
    if (!draftShifts.length || isExportingPdf) return

    setIsExportingPdf(true)
    try {
      const cycleStart = (cycleObj?.start_date || '').split(' ')[0].split('T')[0]
      const cycleEnd = (cycleObj?.end_date || '').split(' ')[0].split('T')[0]

      if (formatType === 'daily_list') {
        exportAutoGenerateDailyListPdf({
        ...
        toast({
          title: 'PDF exportado (Lista por dia)',
          description: `${filteredDraftShifts.length} plantão(ões) exportados com COREN.`,
        })
      } else {
        // Formato Calendário
        const daysList: Array<{ date: Date; key: string; dayOfWeek: number }> = []
        if (cycleStart && cycleEnd && cycleStart <= cycleEnd) {
          let curStr = cycleStart
          while (curStr <= cycleEnd) {
        ...
        toast({
          title: 'PDF exportado (Calendário)',
          description: 'A grade mensal da escala foi exportada em formato calendário.',
        })
      }
    } catch (error: any) {
      toast({
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Modal de Escolha do Formato de Exportação PDF */}
      <Dialog
        open={isPdfExportModalOpen}
        onOpenChange={(open) => {
          if (!open) setIsPdfExportModalOpen(false)
        }}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Escolha o layout desejado para o documento PDF do rascunho de escala:
            </DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-1 gap-3 py-3">
            <button
              type="button"
              data-testid="btn-pdf-format-daily-list"
              onClick={() => handleExecutePdfExport('daily_list')}
              className="flex items-start gap-3 p-3.5 rounded-lg border border-slate-200 bg-white hover:border-emerald-500 hover:bg-emerald-50/40 text-left transition-all group focus:outline-none focus:ring-2 focus:ring-emerald-500"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">
                  Tabela detalhada agrupada por dia contendo colaborador, COREN, turno e horários de
                  início e fim.
                </p>
              </div>
            </button>
            ...
            <button
              type="button"
              data-testid="btn-pdf-format-calendar"
              onClick={() => handleExecutePdfExport('calendar')}
              className="flex items-start gap-3 p-3.5 rounded-lg border border-slate-200 bg-white hover:border-emerald-500 hover:bg-emerald-50/40 text-left transition-all group focus:outline-none focus:ring-2 focus:ring-emerald-500"
            >
              <div className="p-2 rounded-md bg-emerald-100 text-emerald-700 group-hover:bg-emerald-600 group-hover:text-white transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">
                  Grade mensal em paisagem idêntica ao calendário em tela, com D/N + COREN e
                  destaque de Folga Fim de Semana.
                </p>
              </div>
            </button>
          </div>
          <DialogFooter className="sm:justify-end gap-2">
            <Button
              variant="outline"
              data-testid="btn-cancel-pdf-export"
