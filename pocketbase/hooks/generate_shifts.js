  ...
  (e) => {
    ...
    try {
      ...
      sortedShifts.forEach(function (gs) {
        ...
        // Parity check: dias ímpares não recebem colaborador restrito a pares e vice-versa (com inversão mensal determinística)
        if (uInfo && (uInfo.shift_parity === 'even' || uInfo.shift_parity === 'odd')) {
          if (!isStaffEligibleForCivilDate(dateStr, uInfo.shift_parity)) {
            violations.push(
              'Colaborador ' +
                uInfo.name +
                ' configurado com equipe ' +
                (uInfo.shift_parity === 'even' ? 'Equipe 1' : 'Equipe 2') +
                ' mas plantão em ' +
                dateStr +
